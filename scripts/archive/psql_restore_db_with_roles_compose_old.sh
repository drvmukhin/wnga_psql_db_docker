...
#   /scripts/psql_restore_db_with_roles_compose.sh <target_db> <backup_filename> [role1,role2,...]
#
# Expectations:
...
if [[ -z "$TARGET_DB" || -z "$BACKUP_FILE" ]]; then
  echo "ERROR: Usage: $0 <target_database_name> <backup_filename> [role1,role2,...]" >&2
  exit 1
fi
